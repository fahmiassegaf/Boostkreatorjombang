...
import { initAuth, googleSignIn, getAccessToken } from '../lib/firebase-auth';
...
export default function RegistrationForm() {
  ...
  const appendToSpreadsheet = async (token: string) => {
    ...
    try {
      const response = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range}:append?valueInputOption=${valueInputOption}`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error('Failed to append to spreadsheet:', errData);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (token) {
        await appendToSpreadsheet(token);
      }
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-12 flex flex-col lg:flex-row gap-16 lg:gap-24">
         ...
         >
            ...
            <div className="pt-4 flex flex-col gap-4">
              ...
              <button type="submit" disabled={isSubmitting} className="mt-4 inline-flex h-[56px] items-center justify-center px-8 text-[15px] font-medium text-[#050505] bg-[var(--color-text-main)] rounded-full hover:bg-[var(--color-accent)] transition-all duration-300 w-max disabled:opacity-70 disabled:cursor-not-allowed">
                {isSubmitting ? (
